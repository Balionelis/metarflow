...
struct MetarInfo {
    ...
    date_time: String,
    zulu_day: Option<u32>,
    zulu_hour: Option<u32>,
    zulu_minute: Option<u32>,
    wind: String,
    visibility: String,
    weather: String,
    clouds: String,
    ...
    dewpoint: String,
    altimeter: String,
    altimeter_hpa: Option<u32>,
    altimeter_inches: Option<f32>,
    altimeter_default_unit: String,
    remarks: String,
    raw: String,
}
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/metarflow.svg", get(favicon))
        .with_state(state);
    ...
    while i < parts.len() {
        ...
        if part.starts_with("A") && part.len() == 5 {
            if let Ok(inches_raw) = part[1..].parse::<f32>() {
                let inches = inches_raw / 100.0;
                let inches_str = format!("{:.2}", inches);
                let hpa = (inches * 33.8639) as u32;
...
// formats stat value with empty class if needed
fn format_stat_value(value: &str, default: &str) -> (String, String) {
    if value.is_empty() {
...
<body>
    ...
    <script>
        ...
        // applies dark mode on page load
